...
select e1.emp_name,e2.emp_name,datediff(day,e1.dob,e2.dob) as age_diff
from employee e1
...
select distinct o.sub_category,r.Return_Reason
from orders o
...
select order_id,count(Product_ID) as ordered_products
from orders
group by order_id 
...
select order_id,order_date,ship_date,datediff(day,order_date,ship_date)-2*
datediff(week,order_date,ship_date) as difference_in_days from orders

--6- write a query to print 3 columns : category, total_sales and (total sales of returned orders)

select o.category,sum(o.sales) as total_sales,sum(case when r.order_id is not null then sales end)
as return_orders_sales
from Orders o
left join returns$ r on o.Order_ID = r.Order_Id
group by o.Category
...
select category, sum(case when Datepart(year,order_date) = 2019 then sales end) as total_sales2019,
sum(case when Datepart(year,order_date) = 2020 then sales end) as total_sales2020
from orders
group by category

--8- write a query print top 5 cities in west region by average no of days between order date and ship date.
select top 5 city, avg(datediff(day,order_date,ship_date)) as num_of_days
from orders
where region = 'west'
group by city
